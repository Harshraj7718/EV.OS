...
import { KYC_DOCUMENT_TYPES, type KycDocumentType, type RiderProfile } from "@/lib/rider/types";
...
const KYC_HELP: Record<RiderProfile["kyc_status"], string> = {
  NOT_STARTED: "Submit your driving license or another identity document below to begin verification.",
...
export default function RiderKycPage() {
  ...
  const [documentType, setDocumentType] = useState<KycDocumentType>("DRIVING_LICENSE");
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setNotice(null);
    try {
      await riderApi.submitKycDocument({ document_type: documentType, file_reference: fileReference });
      const updatedProfile = await riderApi.getProfile();
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-brand-900/60">
        Complete identity and driving license verification. No real document storage or
        verification service is connected yet — this is a development/mock flow that only records
        metadata.
      </p>
      ...
      <form onSubmit={handleSubmit} className="mt-6 max-w-md rounded-2xl border border-brand-100 bg-white p-5">
        ...
        <div className="mt-4">
          ...
          <input
            ...
            placeholder="e.g. mock://driving-license.pdf"
